...
def passwordToKey_encrypt(password):
    md5 = hashlib.md5()
    md5.update(password)
    key = []
    for c in list(md5.digest()):
        key.append(ord(c))   
    rand_key=getrandbits(128)    
    l=bit_list(rand_key)
    rsa_encrypt(rand_key)
    #print "Message:",m,"\n Number:",n,"\nKey:",d
    key.extend(l)
    return key


def passwordToKey_decrypt(password,l):
    md5 = hashlib.md5()
    md5.update(password)
    key = []
    for c in list(md5.digest()):
        key.append(ord(c))   
    #l=bit_list(getrandbits(128))
    #l=[221L, 152L, 197L, 117L, 177L, 55L, 156L, 97L, 60L, 108L, 255L, 177L, 146L, 239L, 225L, 248L]
    #print l 
    key.extend(l)
    return key
...
def encrypt(myInput, password, outputfile=None):
    ...
    aesKey = passwordToKey_encrypt(password)
...
def decrypt(myInput, password,outputfile=None):
    ...
    message=long(raw_input("Enter the 256-bit encrypted key"))
    n=long(raw_input("Enter the public key n "))
    d=long(raw_input("enter the public key d"))
    rand_key=rsa_decrypt(message,n,d)
    # convert password to AES 256-bit key
    aesKey = passwordToKey_decrypt(password,bit_list(rand_key))
...
def run_encrypt():
    ...
    outputfile = raw_input("File to output: ")
    password =get_NonVolatile()
    ...
    f=open(outputfile,'r')
    stream=f.read()
    stream+='\0' # adding delemiter
   # print stream
    modify_volatile(get_bitstream_str(stream)) 
...
#    print "done done done "
    inputfile = open('testans','w')

    for c in block:
        inputfile.write(chr(c))
...
if __name__ == "__main__":
    ...
    k=int(raw_input("1.Encrpyt data and Hide it in a Image \n2.To obtain data hidden in a image and decrypt it:"))
    if k==1:
        run_encrypt()
	print "Data is embeded in the Image"
    else:
        #rand_key=long(raw_input('Enter the 128 bit key obtained from RSA :P ' ))
        run_decrypt()

    #rand_key=[96L, 86L, 240L, 36L, 75L, 142L, 11L, 2L, 60L, 118L, 187L, 181L, 81L, 47L, 239L, 86L]
    #run_decrypt(rand_key)
